...
export function buildConsolidationArgs(prompt: string, model?: string): string[] {
  return [
    "-p", prompt,
    "--print",
    "--no-extensions",
...
export default function (pi: ExtensionAPI) {
  ...
  async function consolidateSession(): Promise<void> {
    ...
    try {
      const execPromise = pi.exec("pi", buildConsolidationArgs(prompt, injectorConfig.consolidationModel), {
