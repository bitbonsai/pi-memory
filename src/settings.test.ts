...
let globalConfig: ReturnType<typeof readSettingsConfig>;
...
describe("readSettingsConfig", () => {
  before(() => {
    ...
    globalConfig = readSettingsConfig();
  });
  ...
  it("keeps global model when local consolidationModel is invalid", () => {
    writeProjectSettings({ memory: { consolidationModel: 42 } });
    const cfg = readSettingsConfig(tmpProject);
    assert.equal(cfg.consolidationModel, globalConfig.consolidationModel);
  });

  it("keeps global model when local consolidationModel is empty", () => {
